...
string doc_classification=this.getProperty("classification");
if(string.IsNullOrEmpty(doc_classification))
{string sql2 = $"update innovator.Document set name=N'{doc_classification}' where id='{itemID}'";
        inn.applySQL(sql2);}
...
string doc_classification=this.getProperty("classification");
string doc_name=this.getProperty("name");
...
{          
           string affiliated_companies = this.getProperty("affiliated_companies");
               if (affiliated_companies=="熠芯科技")
               { 
              
                item_number= item_number.Substring(2,item_number.Length-5);
                item_number="YX"+item_number;
                 string sqls = $"update innovator.Document set item_number=N'{item_number}' where id='{itemID}'";
                return inn.applySQL(sqls);
               }     
          return this;    
}
else
{
 return inn.newError("编号领取错误，无法进行正确编码");
    
}
}
 catch (Exception ex)
{
    throw new Exception("Error_model:"+ex.Message); 
}
finally
{
    if (PermissionWasSet) Aras.Server.Security.Permissions.RevokeIdentity(plmIdentity);
...
{
    string classification=this.getProperty("classification").Trim();
    if (classification=="1 芯片/5 发布阶段/2结项")
    {
        return this;
    }
 return inn.newError("编号领取错误，无法进行正确编码");
